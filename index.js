...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ NEW MEMBER IN GROUP _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nSELAMAT BERGABUNG\n *_____________*\nMoga betah Di group Dan Jangan Lupa Intro Yha Dan Jangan Lupa subscribe Maslent YT yha Heh!`
				...
				teks = `hey kak @${num.split('@')[0]} Ichi Titip Gorengan Yah Btw Gausa Kembali Lagi... Kembali = kick 😋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[WAIT] BOT IS PROCESSING 🍥PATIENCE IS THE KEY!',
				asik: '[WAIT] BOT IS PROCESSING 🍥PATIENCE IS THE KEY!',
				...
				only: {
					...
					userB: `──「 REGISTER 」──\nHello User !\nSorry...User..You Have Not Registered In This DataBase.. Please Register To Continue By Using This Command,\n\nCommand : ${prefix}daftar name|age\nContoh : ${prefix}daftar ichi|17\n\n──「 LC BOT 」──`,
			...
			const ownerNumber = ["919072791067@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"919072791067@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["919072791067@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["919072791067@s.whatsapp.net"]//ubah nomor lo
			const premium = ["919072791067@s.whatsapp.net","919072791067@s.whatsapp.net"]
		...
		case 'iri':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                 ...
                 client.sendMessage(from, 'kenalin ya itu owner ku >_-',MessageType.text, { quoted: mek} )
                 tod = await getBuffer(`https://i.ibb.co/XFrR3Qj/637cb6919692.jpg`)
                 client.sendMessage(from, tod, image, { quoted: mek, caption: '*no cuple no smile bro!*'})
                 break
                 case 'emailtome':
            try {
            	if (isBanned) return reply(mess.only.benned)    
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`Registered  Successfully SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`On ${date} ${time}\`\`\`\n\`\`\`[Name]: ${jeneng}\`\`\`\n\`\`\`[Number]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Age]: ${umure}\`\`\`\n\`\`\`To Use The Bot\`\`\`\n\`\`\`Please\`\`\`\n\`\`\`Send ${prefix}help\`\`\`\n\`\`\`\nTotal User ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'testime':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
          ...
          contextInfo: { mentionedJid: [groupOwner] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
                ...
                if (!isUser) return reply(mess.only.userB)
                reply(mess.wait)
                anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(6)}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
 ...
 if (!isOwner) return reply(mess.only.ownerB)
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					case 'lava':
					if (!isUser) return reply(mess.only.userB)
					if (args.length < 1) return reply('Text is Where')
					aruga = body.slice(6)
					reply(mess.wait)
				...
				case 'luxury':
					...
					aruga = body.slice(8)
					reply(mess.wait)
					aruga = await getBuffer(`https://arugaz.my.id/api/textpro/luxury?text=${aruga}`)
					client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
					break
			 case 'text3d':
					if (!isUser) return reply(mess.only.userB)
					...
					aruga = body.slice(8)
					reply(mess.wait)
					aruga = await getBuffer(`https://arugaz.my.id/api/textpro/realvintage?text=${aruga}`)
					client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
					break
					case 'realcloud':
					if (!isUser) return reply(mess.only.userB)
					...
					aruga = body.slice(8)
					reply(mess.wait)
					aruga = await getBuffer(`https://arugaz.my.id/api/textpro/glue3d?text=${aruga}`)
					client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
					break
					case 'captamerica':
					if (!isUser) return reply(mess.only.userB)
					...
					aruga = body.slice(12)
					reply(mess.wait)
					aruga = await getBuffer(`https://arugaz.my.id/api/textpro/captamerica?text=${aruga}`)
					client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
					break
					case 'toxictext':
					if (!isUser) return reply(mess.only.userB)
					...
					aruga = body.slice(10)
					reply(mess.wait)
					aruga = await getBuffer(`https://arugaz.my.id/api/textpro/neonlight?text=${aruga}`)
					client.sendMessage(from, aruga, image, {caption: 'Nih kak', quoted: mek})
					break
				case 'leave': 
				if (isBanned) return reply(mess.only.benned)    
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Gopay :* _08165466368_\n*Saweria :* _https://saweria.co/ICHIbot2412\n*Dana :* _08165466368_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
						...
						res = await fetchJson(`https://api.vhtear.com/randomloli&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomwibu&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				if (!isUser) return reply(mess.only.userB)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/mlherolist?apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://arugaz.my.id/api/media/ytaudio?query=${aruga}`)
					if (anu.error) return reply(anu.error)
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (data.error) return reply(data.error)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=hewanimutlucu&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/tebakgambar&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/funkuis&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					setTimeout( () => {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/family100&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					setTimeout( () => {
					client.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/branly?query=${body.slice(9)}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=elang&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=unta&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cewekcantik&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=cowokganteng&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=lolikawai&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=waifukawai&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					reply(mess.asik)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=nekoanimekawai&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					var mi = JSON.parse(JSON.stringify(anu.result));
					var ku =  mi[Math.floor(Math.random() * mi.length)];
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isNsfw) return reply('❌ *FALSE* ❌')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animehentai&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pin}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					reply(mess.wait)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					tulis = body.slice(6)
					reply(mess.wait)
					buffer4 = await getBuffer(`https://api.vhtear.com/write?text=${tulis}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan, Maksimal 10 kalimat', text, {quoted: mek})
                    buffer6 = await getBuffer(`https://api.vhtear.com/lovemessagetext?text=${teks}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${ige}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/url2image?tipe=${args[0]}&url=${args[1]}&apiKey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					...
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/logoff?hero=alok&text=${tels}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					buffer = await getBuffer(anu.result)
